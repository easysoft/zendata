...
func main() {
	...
	db := comm.GetDB()
	db.AutoMigrate(
		&model.DataOccupation{},
	)

	pos := make([]model.DataOccupation, 0)
	db.Find(&pos)
	...
	db.Raw("desc " + model.DataOccupation{}.TableName()).Scan(&infos)

	excelColNameArr, excelColNameHeader := comm.GetExcelColsByTableDef(infos)
	fieldNames := comm.GetStructFields(model.DataOccupation{})
